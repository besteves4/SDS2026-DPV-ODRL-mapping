...
and it is available at \url{https://w3id.org/dpv/mappings/odrl},
...
The following alignment strategies were considered, with
Table~\ref{tab:mapping} providing an overview of these strategies,
...
to specific data types or demographic groups, e.g.,
to allow only the usage of health-related data from a personal data catalog, or
...
is available at \url{https://w3id.org/dpv/mappings/odrl}
...
\paragraph{Rules and Agreements}

DPV rule concepts align with the core rule types in ODRL.
Permissions, prohibitions, and obligations
can therefore be represented consistently across both vocabularies.
Listing~\ref{lst:rule} provides an example of a
DPV-ODRL mapping for a rule.
In this case,
\texttt{dpv-odrl:Obligation} is equivalent to \texttt{dpv:Obligation},
and closely matches \texttt{odrl:Duty}.
Additionally,
DPV contractual clauses and legal agreements
can be represented as subclasses of ODRL policies.
Examples include confidentiality agreements, 
non-disclosure agreements (NDA), and data handling clauses.
Listing~\ref{lst:policy} provides
the DPV-ODRL mapping for the \texttt{NDA} concept.
In this case,
\texttt{dpv-odrl:NDA},
equivalent to \texttt{dpv:NDA},
is a subclass of \texttt{odrl:Policy},
and mutually exclusive from all ODRL policies and
also from those defined in the DPV-ODRL profile.
The latter condition is required by the 
ODRL Profile Mechanism~\cite{steidl_odrl_2023}
for the definition of additional policy subclasses.

\begin{scriptsize}
\begin{lstlisting}[language=turtle, caption=DPV-ODRL mapping for the Obligation concept., label=lst:rule]
...
\begin{scriptsize}
\begin{lstlisting}[language=turtle, caption=DPV-ODRL mapping for the NDA concept., label=lst:policy]
...
\subsection{Usage of ODRL Operators}
\label{sec:operators}

Besides specifying the mapping
between DPV and ODRL,
when defining the DPV-ODRL profile,
the usage of ODRL operators for
each left operand should also be
included in the left operand specification.
For this,
the \texttt{skos:note} property
can be used to associate a certain left operand
with a note on the operators that must be used
with said left operand.
Listing~\ref{lst:note} provides an example
of such a specification.
In this case,
the \texttt{dpv-odrl:LegalBasis} left operand
can only be used with the
\texttt{odrl:isA},
\texttt{odrl:eq}, and
\texttt{odrl:neq} operators.
Section~\ref{sec:shacl} will provide details
on how to implement this specification.

\begin{scriptsize}
\begin{lstlisting}[language=turtle, caption=Note specification for the usage of operators with the Legal Basis left operand., label=lst:note]
dpv-odrl:LegalBasis a odrl:LeftOperand, skos:Concept ;
	skos:exactMatch dpv:LegalBasis ;
  	skos:example "A legal basis can be consent, legitimate interest, contract performance or jurisdiction-specific legal basis such as the ones provided in GDPR Article 6."@en ;
